...
export function StudentEvaluationDialog({ isOpen, onOpenChange }: StudentEvaluationDialogProps) {
  const { user } = useAuth();
  const { handleEvaluateStudent } = useStudentProfile();
  ...
  const onSubmit = async (data: EvaluationFormValues) => {
    ...
    const success = await handleEvaluateStudent(evaluationData);
    setIsLoading(false);
    if (success) {
      form.reset();
      onOpenChange(false);
