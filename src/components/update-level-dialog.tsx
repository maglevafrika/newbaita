...
export function UpdateLevelDialog({ isOpen, onOpenChange, currentLevel }: UpdateLevelDialogProps) {
  const { handleUpdateStudentLevel } = useStudentProfile();
  ...
  const onSubmit = async (data: LevelFormValues) => {
    setIsLoading(true);
    const success = await handleUpdateStudentLevel(data.level, data.review);
    setIsLoading(false);
    if (success) {
      form.reset();
      onOpenChange(false);
